...
- `⚙️ Настройки` - хранение сообщений, расшифровка голосовых, режим ответа реплаем, префикс расшифровки и автоудаление ваших voice.
...
В `⚙️ Настройки` можно включить `🧽 Удалять мои voice`. Тогда, если расшифровка включена и вы отправили голосовое в Business-чате, бот скачает аудио, сразу попробует удалить исходное голосовое, а потом отправит текстовую расшифровку. Для этого в Telegram Business у бота должны быть права на удаление сообщений.

Там же можно выключить заголовок `Расшифровка:` полностью или изменить его текст через кнопку `✏️ Изменить префикс`.

Также в `⚙️ Настройки` есть улучшение текста расшифровки:

- `🪄 Улучшать текст` - чистит мусор распознавания, пробелы, пунктуацию и применяет словарь замен.
- `🤬 Мат/разговорность` - если включено, бот не смягчает мат и сохраняет разговорную речь ближе к оригиналу.
- `✨ Эмодзи по смыслу` - добавляет 1-3 подходящих эмодзи в конец расшифровки по общему смыслу сообщения.
- `🧾 Словарь автозамен` - ваши правила вроде `серега => Серёга`, `open ai => OpenAI`.
