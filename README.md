...
- `🗄 Архив` - выгрузка HTML-архива и быстрый переход к очистке.
- `🛟 Поддержка` - VPN-автоответы, обучение, анализ переписки, название сервиса и контакт.
- `🧹 Обслуживание` - удаление вложений и ZIP-выгрузок за период или полностью.
- `📊 Статистика` - сообщения, голосовые, вложения, чаты и топ активных чатов.
- `⚙️ Настройки` - хранение сообщений, расшифровка голосовых и режим ответа реплаем.
...
- `learned_replies.csv` и `learned_replies.jsonl` - база выученных ответов поддержки.
...
В разделе `Обслуживание` можно освободить место:

- удалить вложения старше 30 дней;
- удалить вложения старше 90 дней;
- удалить все вложения;
- удалить ZIP-выгрузки старше 30 дней;
- удалить все ZIP-выгрузки.

Удаление вложений не удаляет сообщения из базы. В отчёте останется текст и метаданные, но медиафайлы, которые были удалены, уже нельзя будет воспроизвести.
...
Обучение поддержки тоже выключено по умолчанию. Когда оно включено, бот анализирует сохранённую переписку: ваши ручные ответы реплаем, последовательности клиент->оператор и типовые инструкции оператора. При следующем похожем вопросе VPN-поддержка сначала попробует использовать выученный ответ, а потом уже обычные шаблоны.

В разделе `🛟 Поддержка` можно без SSH менять:

- название сервиса;
- контакт поддержки;
- включение автоответов;
- включение обучения;
- принудительный анализ всей накопленной переписки;
- сброс базы выученных ответов.
