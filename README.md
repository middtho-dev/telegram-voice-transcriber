...
- `Поддержка` - VPN-автоответы, обучение на ваших реплаях и сброс базы обучения.
...
- `Настройки` - хранение сообщений, расшифровка голосовых и режим ответа реплаем.
...
Обучение поддержки тоже выключено по умолчанию. Когда оно включено, бот учится только на ваших ручных ответах реплаем в Business-чате: клиент написал вопрос, вы ответили на него реплаем, бот запомнил эту пару. При следующем похожем вопросе VPN-поддержка сначала попробует использовать выученный ответ, а потом уже обычные шаблоны.
